...
파일 복사와 작업 스케줄러 등록이 자동으로 완료됩니다. 별도의 비밀번호 입력은 필요하지 않습니다.
...
SteamVR이 레이아웃을 추가하는 데 시간이 걸리므로, 서비스는 steamtours.exe 감지 후 10초 뒤에 제거를 시도합니다. 이벤트 뷰어에서 오류가 없다면 정상입니다.

**Q. 프로그램이 실행되지 않습니다.**

작업 스케줄러(taskschd.msc)에서 SteamVRKeyboardFix 항목을 확인하세요. 없으면 MSI를 재설치하세요.
...
File copy and Task Scheduler registration are handled automatically. No password entry is required.
...
SteamVR adds the layout gradually as it connects to the HMD. The service waits 10 seconds after detecting steamtours.exe before attempting removal. Check Event Viewer for any error entries.

**Q. The program does not run.**

Check Task Scheduler (taskschd.msc) for the SteamVRKeyboardFix entry. If missing, reinstall the MSI.
...
ファイルのコピーとタスクスケジューラへの登録が自動的に完了します。パスワードの入力は不要です。
...
SteamVR が HMD に接続しながらレイアウトを追加するため、サービスは steamtours.exe 検出から 10 秒後に削除を試みます。イベントビューアーにエラーがなければ正常動作しています。

**Q. プログラムが起動しません。**

タスクスケジューラ（taskschd.msc）で SteamVRKeyboardFix エントリを確認してください。なければ MSI を再インストールしてください。
...
```csharp
// 1. 감시할 프로세스 이름
private const string TargetProcessName = "steamtours.exe";  // ← 여기를 변경
...
WHERE ProcessName = 'steamtours.exe' OR ProcessName = 'vrdashboard.exe'
```
...
private const string TargetProcessName = "steamtours.exe";  // ← change here
...
```csharp
// 1. 監視するプロセス名
private const string TargetProcessName = "steamtours.exe";  // ← ここを変更
...
| 2000 | steamtours.exe 감지 | steamtours.exe detected | steamtours.exe 検出 |
